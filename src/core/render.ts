...
import { player, portal, platforms, clouds } from '../state.ts'
...
export function createRender(renderer: ReturnType<typeof createRenderer>, canvas: HTMLCanvasElement) {
  return () => {
    ...
    const sprites: RenderSprite[] = platforms.map((p) => ({ x: p.x + offsetX, y: p.y + offsetY, rotation: 0, r: p.r, r2: p.r2, cell: p.cell }))
    if (!night) clouds.forEach((c) => sprites.push({ x: c.x, y: c.y, rotation: 0, r: c.r, r2: c.r2, flip: c.flip, cell: CLOUD_CELL }))
    sprites.push({ x: portal.x + offsetX, y: portal.y + offsetY + portal.oy, rotation: 0, r: portal.r, r2: portal.r2, cell: portal.cell })
    sprites.push({
      x: player.x + offsetX,
      y: player.y + offsetY + (player.oy ?? 0),
